			 ...
			 Else
					If PayTF="yes" Then
						...
						Select Case ModelChargeType
					     ...
					     Call KSUser.UseLogConsum(3,ChannelID,ID,RSObj("Title"))
					Else
						ShowInfoStr="<div align=center>下载本软件需要消耗 <font color=red>" & ReadPoint & "</font> " & ChargeStrUnit &",你目前尚有 <font color=green>" & CurrPoint & "</font> " & ChargeStrUnit &"可用,下载本" & KS.C_S(ChannelID,3) & "后，您将剩下 <font color=blue>" & CurrPoint-ReadPoint & "</font> " & ChargeStrUnit &"</div><div align=center>你确实愿意花 <font color=red>" & ReadPoint & "</font> " & ChargeStrUnit & "来下载本" & KS.C_S(ChannelID,3) & "吗?</div><div>&nbsp;</div><div align=center><a href=""?m=" &ChannelID & "&ID=" & ID & "&PayTF=yes&DownID=" & DownID & """>我愿意</a>    <a href=""" &DomainStr & """>我不愿意</a></div>"
					End If
			 End If
	   End Sub
	   Sub GetNoLoginInfo()
		   ShowInfoStr="<div align=center>对不起，你还没有登录，本" & KS.C_S(ChannelID,3) & "至少要求本站的注册会员才可下载!</div><div align=center>如果你还没有注册，请<a href=""" & DomainStr & "?do=reg""><font color=red>点此注册</font></a>吧!</div><div align=center>如果您已是本站注册会员，赶紧<a href=""" & domainstr & "user/login/""><font color=red>点此登录</font></a>吧！</div>"
	   End Sub
	   Sub GetContent()
