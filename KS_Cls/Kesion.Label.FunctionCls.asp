...
Class RefreshFunction
		...
		Function Parse(sTemplate, iPosBegin)
			...
			select case Lcase(sTemp)
			 ...
			 case "photourl","userface" if KS.IsNul(Node.SelectSingleNode("@" & Lcase(sTemp)).text) then echo DomainStr & "images/nopic.gif" Else echo Node.SelectSingleNode("@" & Lcase(sTemp)).text
			 ...
			 case "price_member","price_original","price_market" echo formatnumber(Node.SelectSingleNode("@" & lcase(sTemp)).text,2,-1,-1)
		   ...
		   If ModelID="0" Then 
			...
			Else 
			 ...
			 Select Case KS.C_S(ModelID,6) 
			  ...
			  Case 5  
			   FieldStr  = "I.ID,I.Title,I.Tid,I.Inputer,I.Fname,I.AddDate,I.Popular,I.Hits"
			   If PrintType>=2 Then FieldStr=FieldStr & ",I.PhotoUrl,I.ProIntro As Intro,I.BigPhoto,I.Price_market,I.Price_member,I.Price,I.Price_Original,I.Discount,I.Promodel,I.Point,I.BrandID"
	  ...
	  End Function
	   ...
	   Function ExplainRollsLabelBody()
		...
		Function GetNavigation(LabelStyle)
			 ...
			 If Cint(P_T)=3 Then
			  FieldStr="ClassID,ID,ID AS Tid, FolderName,TS,ClassBasicInfo"
			 Else
			  FieldStr="ClassID,ID,FolderName,TS"
			 End If
			   ...
			   case "9999" 
			       if FCls.RefreshFolderID="0" then
					   SqlStr = "Select " & FieldStr & ",TN,FolderOrder From KS_Class a inner join KS_Channel b on a.channelid=b.channelid Where  B.ChannelStatus=1 and TN='0' AND TopFlag=1 And DelTF=0 Order By root,FolderOrder"
					    ...
					    SqlStr = "Select " & FieldStr & " From KS_Class A Inner Join KS_Channel B On A.ChannelID=B.ChannelID Where B.ChannelStatus=1 And TN='" & FCls.RefreshFolderID & "' And DelTF=0  Order BY root,FolderOrder"
					end if
			   case "9998"
					  Dim Rst ,ParentID
		...
		Function GetAQList(LabelStyle)
			 ...
			 End If
 			 ...
 			 Dim SqlStr,RecommendTF,RewardTF,ZeroTF,OrderStr,JoinType
			 ...
			 If ParamNode.getAttribute("printtype")="2" or Lcase(ParamNode.getAttribute("showuserface"))="true" Then
			  ...
			  SqlStr="Select TOP " & num & " a.TopicID,a.ClassID as AqClassId,a.UserName,a.ClassName as AQClassName,a.Title,a.DateAndTime as AddDate,a.hits,a.reward,a.Anonymous,a.LastPostTime,a.ExpiredTime,b.userface From KS_AskTopic a " & JoinType &" KS_User b On A.userName=B.userName" & Param 
			 Else
		...
		Function GetSpaceList(LabelStyle)
			...
			Dim SqlStr:SqlStr="Select TOP " & num & " b.userid,b.username,b.blogname,[Domain],b.logo,b.banner,b.hits From KS_Blog B " & Param
		...
		Function GetBlogInfoList(LabelStyle)
			 ...
			 Param=" Where istalk<>1 and Status=0"
			 ...
			 Param=Param & " order by " & OrderStr
			 Dim SqlStr:SqlStr="Select TOP " & num & " ID,UserID,Title,UserName,AddDate,TypeID From KS_BlogInfo " & Param
			Dim RS:Set RS=Conn.Execute(SqlStr)
			If Not RS.Eof Then Set XMLSql=KS.RsToXml(RS,"row","root") Else XMLSql=Empty
			RS.Close:Set RS=Nothing
			If IsObject(XMLSql) Then
