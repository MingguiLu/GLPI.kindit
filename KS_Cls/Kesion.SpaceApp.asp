...
Class SpaceApp
		...
		'============================结束记录============================================================
		 ...
		 select case Lcase(action)
		   ...
		   case "info"
		     KSBCls.Title="资料"
			 substr=substr & KSBcls.Location("<strong>首页 >> 个人档</strong>")
		...
		Sub BlogLog()
		  ...
		  If RS("IsTalk")<>"1" Then
		  substr=substr & KSBcls.Location("<strong>首页 >> 查看" & UserName & "发表的博文</strong>")
		  Else
		  substr=substr & KSBcls.Location("<strong>首页 >> 查看" & UserName & "发表的新鲜事</strong>")
		  End If
		  ...
		  Else
			  SubStr=substr & LFCls.GetConfigFromXML("space","/labeltemplate/label","log")
			   ...
			   End If
				Dim MoreStr:MoreStr="阅读次数("&RS("hits")&") | 回复数("& Conn.Execute("Select Count(ID) From KS_BlogComment Where LogID="  &id)(0) &")</div>"
				Dim ContentStr
...
End Function
	 ...
	 Sub ClubList()
	     MaxPerPage=20
	     substr=substr & KSBcls.Location("<strong>首页 >> 我的论坛话题</strong>")
		 Dim RS:Set RS=Server.CreateObject("ADODB.RECORDSET")
		 ...
		 Else
				...
				SubStr=SubStr & "<td align=""center"">主题</td><td align=""center"">版块</td>"
				SubStr=SubStr & "<td align=""center"">回复</td><td align=""center"">最后发表</td></tr>"
				Do While Not RS.Eof
				   SubStr=SubStr & "<tr><td class='splittd'><img src='../images/arrow_r.gif' align='absmiddle' /> <a href='" &KS.GetClubShowUrl(rs("id")) & "' target='_blank'>" & replace(replace(rs("subject"),"{","｛"),"}","｝") & "</a><br/><span class='tips'>发表时间：" & rs("addTime") & "</span></td>"
	 ...
	 '新鲜事列表
	 Sub FreshList()
	     substr=substr & KSBcls.Location("<strong>首页 >> 新鲜事</strong>")
		 If KS.C("UserName")=UserName Then
		   ...
		   If Not RSU.Eof Then
			   ...
			   substr=substr &"<div style='text-align:left;color:#888;font-weight:bold'><img src='" & myface & "' align='left' class='faceborder' width='42' height='42' style='margin-right:4px'/>说说正在发生的事..."
			   substr=substr &"<br/><textarea name=""CommentContent"" id=""CommentContent"" cols=""60"" class=""freshtextarea"" onfocus=""freshFocus('CommentContent');"" onblur=""freshBlur('CommentContent');"" rows=""4"">随便说点什么，让好友们知道你的心情、在做什么……（最少10个字）</textarea><div style=""margin:5px;margin-left:53px;padding-bottom:6px;""><input type='button' value=' 发 表 ' onclick=""postsay()"" class='btn'/></div></div>"
		   End If
		 ...
		 End If
		  ...
		  SubStr=SubStr &">好友新鲜事</a> <a href='../space/?" & userid &"/fresh/2/1'"
		  If ShowType="2" Then SubStr=subStr &" class=""curr"""
		  SubStr=SubStr &">" & UserName & "的新鲜事</a></div>"
		  Else
		   ShowType=2
		  End If
		  ...
		  RSObj.Open SQLStr,Conn,1,1
		   If RSObj.EOF and RSObj.Bof  Then
		     If ShowType=2 Then
		     SubStr=SubStr & "<div>这家伙很懒，什么也没有写！</div>"
			 else
		     SubStr=SubStr & "<div>这家伙的好友都很懒，什么也没有写！</div>"
			 end if
		   Else
						   ...
						   If CurrPage > 1 and (CurrPage - 1) * MaxPerPage < totalPut Then
							...
							End If
								...
								Do While Not RSObj.Eof
									 ...
									 substr=substr & " <a href=""javascript:void(0)"" onclick=""showcmt(" & rsobj("id") & ")""> 评论" & CmtNumStr & "</a>"
									 
									 If KS.C("UserName")=UserName And ShowType="2" Then
									   substr=substr & " <a href=""" & KS.GetDomain & "user/user_fresh.asp?action=delfresh&username=" & server.URLEncode(username) &"&id=" & rsobj("id") &""" onclick=""return(confirm('确定删除该新鲜事吗？'))"" style=""color:#ff6600"">[删除]</a>"
									 End If									 
									 ...
									 If CmtNum>0 Then
									   ...
									   If Not RSC.Eof Then 
										   ...
										   substr=substr & "<tr><td class='splittd' colspan='2'>此条新鲜事共有 <span style='color:red'>" & CmtNum & "</span> 条评论，<a href='../space/?" & uid & "/log/" & KS.ChkClng(rsobj("id")) & "' target='_blank'>查看全部...</a></td></tr>"
										   Do While Not RSC.Eof
										    ...
										    substr=substr & "<tr><td valign='top' class='splittd' style='width:50px;text-align:center;margin:5px 2px 2px 0px;'><img class='faceborder' src='" &facestr & "' width='40' height='40'/></td><td class='splittd' style=""width:300px""><a href='" & Urls & "'>" & RealName & "</a> " & KS.LoseHtml(RSC("Content")) & " " & KS.GetTimeFormat(RSC("Adddate")) 
											 ...
											 substr=substr & "<div style=""margin : 5px 20px; border : 1px solid #efefef; padding : 5px;background : #ffffee; line-height : normal;""><b>主人 <a href='../space/?" & userid & "' target='_blank'>" & rname & "</a> 回复:</b><br>" & RSc("Replay") & "<br><div align=right>时间:" & rsc("replaydate") &"</div></div>"
											 End If
									 ...
									 substr=substr &"<textarea name=""Content"" id=""c" &rsobj("id") & """ class=""cmttextarea"" onblur=""ThisBlur(" & rsobj("id") & ")"" onfocus=""ThisFocus(" & rsobj("id") & ")"" cols=""50"" rows=""2"">我也说一句...</textarea><br/><div style=""margin:4px 0px 4px 0px""><input type=""submit"" class=""btn"" onclick=""return(postcmt(" & rsobj("id") & "))"" value=""发表""/></div></form></div>"
	 ...
	 Sub BlogList()
	     Dim Loc
		 If KS.C("UserName")=UserName Then Loc="<span style=""float:right""><a href=""" & KS.Setting(2) & "/user/User_Blog.asp?Action=Add""><img src='" & KS.Setting(2) & "/user/images/icon7.png' border='0'/>写博文</a></span>"
		 Loc=Loc & "<strong>首页 >> 博文</strong>"
		         ...
		         If RSObj.EOF and RSObj.Bof  Then
						...
						Else
						  If KeyWord="" And ClassID=0 Then
							substr=substr & "<div style=""border: #efefef 1px dotted;text-align:center"" colspan=3><p>您还没有写博文博文！</p></div>"
    ...
    Sub showlog(RS)
		 ...
		 Do While Not RS.Eof 
		   ...
		   substr= substr & "<span class='tips'>分类：[<a target='_blank' href='" & KS.Setting(2) & "/space/morelog.asp?classid=" & rs("typeid") &"'>" & rs("typename") & "</a>] <a href=""" & Url  & """>阅读全文("&RS("hits")&")</a>  <a href=""" & Url  & "#Comment"">回复("& Conn.Execute("Select Count(ID) From KS_BlogComment Where LogID="  &RS("id"))(0) &")</a></span>"
           substr=substr &"</div>"
		  RS.MoveNext
		 I = I + 1
		  If I >= MaxPerPage Then Exit Do
	...
	End Sub
	 ...
	 Sub FriendList()
	     MaxPerPage=20
	     substr=substr & KSBcls.Location("<strong>首页 >> " & UserName & "的好友</strong>")
	 ...
	 '圈子列表
	 Sub GroupList()
	     substr=substr & KSBcls.Location("<strong>首页 >> 圈子</strong>")
		 MaxPerPage =10
		 substr=substr &"  <table border=""0"" align=""center"" width=""100%"">" & vbcrlf
		 Dim RSObj:Set RSObj=Server.CreateObject("ADODB.RECORDSET")
	 ...
	 Sub AlbumList()
	     SubStr=SubStr & KSBcls.Location("<strong>首页 >> 相册</strong>")
		 MaxPerPage =9
	...
	'查看相片
	 Sub ShowAlbum()
	   	SubStr=SubStr & KSBcls.Location("<strong>首页 >> 查看相册</strong>")
	 ...
	 Sub GetAlbumBody(xcname)
		...
		'留言
		Sub ShowMessage()
		 SubStr=SubStr & KSBcls.Location("<div align=""left""><strong>首页 >> 留言板</strong>(<a href=""#write"">签写留言</a>)</div>")
		...
		Function GetWriteMessage()
		 ...
		 If KS.SSetting(25)="0" And KS.IsNul(KS.C("UserName")) Then
		  GetWriteMessage="<div style=""margin:20px""><strong>温馨提示：</strong>只有会员才可以留言,如果是会员请先<a href=""javascript:ShowLogin()"">登录</a>,不是会员请点此<a href=""../?do=reg"" target=""_blank"">注册</a>。</div>"
		 Else
		 ...
		 GetWriteMessage = GetWriteMessage & "<tr><td class=""comment_write_title""><span style='font-weight:bold;font-size:14px'>发表您的留言:</span><br/><textarea class=""msgtextarea"" cols='70' rows='4' id=""Content"" onfocus=""if (this.value=='既然来了，就顺便留句话儿吧...') this.value='';"" name=""Content"" onblur=""if (this.value=='') this.value='既然来了，就顺便留句话儿吧...';"">既然来了，就顺便留句话儿吧...</textarea>"
		...
		GetWriteMessage = GetWriteMessage & " <br/>昵称："
		GetWriteMessage = GetWriteMessage & "   <input name=""AnounName"""
		If KS.C("UserName")<>"" Then GetWriteMessage = GetWriteMessage & " readonly"
		GetWriteMessage = GetWriteMessage & " maxlength=""100"" type=""text"" value=""" & KS.C("UserName") & """ id=""AnounName"" style=""background:#FBFBFB;color:#999;border:1px solid #ccc;width:120""/>&nbsp;<font color=red>*</font> 验证码：<input type=""text"" name=""VerifyCode"" onclick=""this.value='';getCode()"" style=""background:#FBFBFB;color:#999;border:1px solid #ccc;width:50px""><span id='showVerify'>鼠标点击输入框获取</span><br/><input type=""submit"" onclick=""return(CheckForm());""  name=""SubmitComment"" value=""OK了，提交留言"" class=""btn""/>"
	...
	End Function
		...
		Next
	    substr= KSBcls.Location("<strong>首页 >> 信息集</strong>")
		Substr=Substr& "<div style='margin:5px'>信息分类<select name='channelid' onchange=""location.href=this.value"">" & opstr & "</select>&nbsp;&nbsp;&nbsp;</div>"
		 MaxPerPage =20
		 substr=substr & "  <table border=""0"" align=""center"" width=""100%"">" & vbcrlf
	...
	'===========9-30========================
		...
		Public Function ShowPage()
				 ...
				 PageStr = ("<div class=""fenye""><table border='0' align='right'><tr><td><div class='showpage' style='height:28px'>")
				   ...
				   pageStr=pageStr & "<a href=""../space/?" & userid & "/" &action & """ class=""prev"">首 页</a>"
