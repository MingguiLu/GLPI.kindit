	  ...
	  '替换频道专用标签
		...
		Do While True 
			...
			If iPosCur>0 Then
					...
					tempstr=tempstr  & "<table border=""0"" class=""annex"" cellspacing=""2"" style=""width:550px;""><tr><td height=""20"" class=""annextitle"">&nbsp;<B>下载信息</B>&nbsp;&nbsp;[文件大小："  & FileSize &" 下载次数：<script src=""" & KS.GetDomain & "item/filedown.asp?action=hits&id=" & FileInfoArr(0) & """></script> 次]"
					tempstr=tempstr & "<tr><td><img src=""" & KS.GetDomain & "editor/ksplus/fileicon/" & Ext & ".gif"" /> <a href=""" & KS.GetDomain & "item/filedown.asp?id=" & FileInfoArr(0) & """ target=""_blank"">" & Title & "</a></td></tr>"
		...
		Function ReplaceEmot(c)
		 ...
		 For K=1 To 70
		  NS=Right("0" & K,2)
		  c=replace(c,"[em"&NS &"]","<img title='" & strarr(k-1) & "' alt='" & strarr(k-1) & "' src='" & KS.Setting(2) & "/editor/ubb/images/smilies/default/" & NS & ".gif'>")
		 Next
		 ReplaceEmot=C
	   End Function
					  ...
					  Select Case ShowStyle
					   ...
					   case 4 '不分页
						...
						For n=1 To TotalPage
						  ...
						  If CurrPage = N Then
						  	ThumbList=ThumbList &"<li><a id=""t" & n & """ class=""currthumb"" href=""javascript:void(0)"" onclick=""showImg(" & n & ");""><img src=""" & Split(ContentArr(n-1),"|")(2) &""" border=""0""/></a></li>"
						  Else
						   ThumbList=ThumbList &"<li><a id=""t" & n & """ class=""normalthumb"" href=""javascript:void(0)"" onclick=""showImg(" & n & ");""><img src=""" & Split(ContentArr(n-1),"|")(2) &""" border=""0""/></a></li>"
						  End If
						Next
					   ...
					   case 2
							...
							For I = 0 To Tpage - 1
								...
								For r=1 to pagenum
									  ...
									  ThumbList=ThumbList&"<li><a id="""" href=""" & Split(ContentArr(n), "|")(1) & """  class=""highslide"" onclick=""return hs.expand(this)"" title=""""><img alt='" & Split(ContentArr(n), "|")(0) & "' src='" & Split(ContentArr(n), "|")(2)  & "' style='border:1px #999999 solid' border='0'></a><div style='text-align:center'>" & KS.Gottopic(Split(ContentArr(n), "|")(0),15) & "</div></li>"
									  else 
									   exit for
									  end if
									  n=n+1
					   ...
					   case 3
							...
							For I = 0 To Tpage - 1
								...
								if CurrPage<=1 then  n=0 else n=pagenum*(CurrPage-1)
								   For r=1 to pagenum
										  if n<=ubound(ContentArr) Then	ThumbList=ThumbList & "<div><img onload=""javascript:resizepic(this)"" alt='" & Split(ContentArr(n), "|")(0) & "' src='" & Split(ContentArr(n), "|")(1)  & "' style='border:1px #999999 solid' border='0'></div><span>" & Split(ContentArr(n), "|")(0) & "</span>"  Else Exit For
										  n=n+1
								   Next
								If CurrPage <> 1 Then FilePathAndName = FilePathAndNameTemp & Fname & "_" & CurrPage & FExt
								F_C = TFileContent
	   ...
	   '截取字符串
		...
		Function GetVote(VoteID)
			...
			if rsVote.bof and rsVote.eof then 
				...
				If IsObject(XML) Then
					if rsVote("VoteType")="Single" then
						for each node in Xml.DocumentElement.SelectNodes("voteitem")
							GetVote=GetVote & "<input type='radio' name='VoteOption' value='"& Node.getAttribute("id") &"'>" & Node.childNodes(0).text &"<br>"&vbcrlf
						Next
						
					else
						for each node in Xml.DocumentElement.SelectNodes("voteitem")
							GetVote=GetVote &  "<input type='checkbox' name='VoteOption' value='"& Node.getAttribute("id") &"'>&nbsp;"& Node.childNodes(0).text &"<br>"&vbcrlf
						next
					end if
			    End If
				...
				GetVote=GetVote &  "<input type='image' src='" & domainStr & "Images/Default/voteSubmit.jpg' border='0'>&nbsp;"&vbcrlf
				GetVote=GetVote &  "<a href='" & domainStr & "index.asp?do=vote&id=" & VoteID &"' target='_blank'><img src='" & domainStr & "Images/Default/voteView.jpg' border='0'></a>"&vbcrlf
		...
		Sub GetTopUser(Num,MoreStr)
		 ...
		 If IsObject(Xml) Then
			For each Node In Xml.DocumentElement.SelectNodes("row")
			  ...
			  echoln "<li><a href=""" & KS.GetDomain & "space/?" & Node.SelectSingleNode("@userid").text & """ target=""_blank"" class=""b""><img src=""" & userface & """ border=""0"" alt=""用户:" & UserName & "&#13;&#10;登录:" & Node.SelectSingleNode("@logintimes").text & "次&#13;&#10;性别:" & Node.SelectSingleNode("@sex").text & """/></a><br /><a class=""u"" href=""" & KS.GetDomain & "space/?" & Node.SelectSingleNode("@userid").text & """ target=""_blank"">" & UserName & "</a></li>"
			Next
