		 ...
		 If ReadPoint<=0 Then Call GetContent():Exit Sub
			 ...
			 Else
					If PayTF="1" Then
					 ...
					 Select Case ModelChargeType
					   case 1
					     If PayPoint>0 Then Call KS.MoneyInOrOut(Node.SelectSingleNode("@inputer").text,Node.SelectSingleNode("@inputer").text,PayPoint,4,1,now,0,"系统",KS.C_S(ChannelID,3) & TcMsg,ChannelID,ID,1)
					     Call KS.MoneyInOrOut(KSUser.UserName,KSUser.UserName,ReadPoint,4,2,now,0,"系统",Descript,ChannelID,ID,1)
						 Call GetContent()
					   case 2
					     If KS.ChkClng(PayPoint)>0 Then Call KS.ScoreInOrOut(Node.SelectSingleNode("@inputer").text,1,KS.ChkClng(PayPoint),"系统",TcMsg,0,0)
