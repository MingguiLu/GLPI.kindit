...
'****************************************************
Class BlogCls
      Public KS,UserName,UserID,Domain,Node,Title
	 ...
	 Function ReplaceBlogLabel(Template)
	  ...
	  Template=Replace(Template,"{$ShowBlogDescript}",Node.SelectSingleNode("@descript").text)
	 ...
	 Function ReplaceMenu() 
	   ...
	   Dim AlbumUrl,GroupUrl,FriendUrl,XXUrl,InfoUrl,FreshUrl
	   If KS.SSetting(21)="1" Then
	  ...
	  if conn.execute("Select top 1 username From KS_enterprise Where UserName='" & UserName & "'").eof Then
	    ReplaceMenu="<div id=""Menu"">"_
					 ...
					 &" <li><a href=""" & FreshUrl & """>新鲜事</a></li>"_
					 &" <li><a href=""" & BlogUrl & """>博文</a></li>"_
					 &" <li><a href=""" & AlbumUrl & """>相册</a></li>"_
	  ...
	  Else
	   	  ReplaceMenu="<div id=""Menu"">"_
					...
					&" <li><a href=""" & GroupUrl & """ title=""公司圈子"">圈子</a></li>" _
					 &" <li><a href=""" & FreshUrl & """  title=""新鲜事"">新鲜事</a></li>"_
	 ...
	 Function ReplaceUserInfoContent(ByVal Content,Flag)
	  ...
	  if left(userfacesrc,1)<>"/" and lcase(left(userfacesrc,4))<>"http" then userfacesrc="../" & userfacesrc
		...
		Else
		 ...
		 Content=Replace(Content,"{$GetSign}",Ubbcode(Sign,0))
		End If
...
on error resume next
		 ...
		 Else
		   ...
		   If IsObject(XML) Then
		    For Each Node In XML.DocumentElement.SelectNodes("row")
			 str=str & "<img src='../images/arrow_r.gif' align='absmiddle'/> <a href='" & KS.GetClubShowUrl(Node.SelectSingleNode("@id").text) & "' target='_blank'>" & Replace(Replace(Node.SelectSingleNode("@subject").text,"{","｛"),"}","｝") &"</a> " & KS.GetTimeFormat(Node.SelectSingleNode("@addtime").text) & "<br/>"
			Next
		   End If
		 End If
		 GetClubTopic=Str
	End Function
	 ...
	 Function GetNewLog()
		 ...
		 if rs.eof then
		   ...
		   do while not rs.eof
			...
			str=str &"<img src=""" & KS.GetDomain & "images/arrow_r.gif"" align=""absmiddle""> <a href='" & url & "' target='_blank'>" & RS("title") & "</a>&nbsp;&nbsp;(" & RS("adddate") & ")<br/>"
			end if
			i=I+1
			str=str &"</td></tr>"
		   rs.movenext
     ...
     Function GetNewXX()
	    Dim SQLStr:SQLStr="Select top 5 ChannelID,InfoID,Title,Tid,Fname,AddDate,Intro from KS_ItemInfo Where Inputer='" & UserName & "' and verific=1 Order By AddDate Desc,id desc"
		Dim Str,I,url
		Str="  <table border=""0"" align=""center"" width=""100%"">" & vbcrlf
				 ...
				 Else 
				   I=0
					do while not rs.eof 
						  ...
						  str=str & "<tr><td class=""splittd""><img src=""" & KS.GetDomain & "images/arrow_r.gif"" align=""absmiddle""> [" & KS.GetClassNP(RS("tid")) & "] <a href='" & url & "' target='_blank'>" & RS(2) & "</a>&nbsp;&nbsp;(" & RS(5) & ")</td></tr>"
						  end if
					      i=I+1
						  rs.movenext
						loop
	...
	End Function
	 ...
	 Function GetUserInfo()
	  ...
	  if not rs.eof then
		...
		' str=str &"<li><a target=blank href=tencent://message/?uin=" & rs(2) &"&Site=" & KS.Setting(2) & "&Menu=yes><img SRC=http://wpa.qq.com/pa?p=1:" & rs(2) & ":5 alt=""点击这里给我发消息"" border=""0""></a>"
		 'else
		 str=str &"<a href=""../space/?" & username & "/info""><img border=""0"" src=""" & KS.GetDomain & "space/images/card.gif"" align=""absmiddle""> 小档案</a>"
		' end if
		 str=str & "</li></div></div>"
	  end if
	  rs.close:set rs=nothing
	 ...
	 Function GetComment()
	  ...
	  If Not RS.Eof Then
	    Do While Not RS.EOF
		 GetComment=GetComment & "<img src=""../images/arrow_r.gif"" align=""absmiddle""> <a title=""" & RS("AnounName") & "发表于" & RS("AddDate")&""" href=""" &GetCurrLogUrl(UserID,RS("LogID")) & "#" & RS("ID") &""">" & KS.Gottopic(KS.LoseHtml(RS("Content")),25) & "</a><br />" & vbcrlf
		RS.MoveNext
		Loop
	  Else
	   GetComment="暂无评论!"
	 ...
	 Function GetMessage()
	  ...
	  RS.Open "Select Top " & MaxTop & " m.*,u.userface,u.userid From KS_BlogMessage m left join KS_User u on m.AnounName=u.username Where m.status=1 and m.UserName='" & UserName & "' Order By m.id Desc",conn,1,1
	  If Not RS.Eof Then Set Xml=KS.RsToXml(rs,"row","")
	  RS.Close:Set RS=Nothing
	  If IsObject(Xml) Then
	  ...
	  End If
         ...
         If KS.SSetting(25)="0" And KS.IsNul(KS.C("UserName")) Then
		  GetMessage=GetMessage & "<div style=""margin:20px""><strong>温馨提示：</strong>只有会员才可以留言,如果是会员请先<a href=""javascript:ShowLogin()"">登录</a>,不是会员请点此<a href=""../?do=reg"" target=""_blank"">注册</a>。</div>"
		 Else
		 ...
		 GetMessage = GetMessage & "<form name=""myform"" action=""../plus/ajaxs.asp?action=MessageSave"" method=""post"">"
	 ...
	 Function Location(str)
	   Location= "<div style=""text-align:left"" class=""location"">"
