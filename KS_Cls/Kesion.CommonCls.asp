...
Class PublicCls
	...
	Sub LoadChannelField()
	  If Not IsObject(Application(SiteSN & "_ChannelField")) then
			Dim Rs:Set Rs = Conn.Execute("Select ChannelID,Title,FieldName,FieldType From KS_Field Order By FieldID")
...
End Function
   ...
   End Function
	...
	End Function
	'论坛分页	
    Function GetClubPageList(MaxPerPage,CurrentPage,TotalPut,ID,PreStatic)
	    dim totalPage
		GetClubPageList= "<table width=""100%"" aling=""center""><tr><td align=right>"
		If Not IsNul(Setting(69)) Then
   ...
   '---------------------------------------------------------------------------------------------------------
	...
	Sub FileAssociation(ByVal ChannelID,ByVal InfoID,ByVal Content,ByVal Flag)
	  ...
	  If Not IsNul(FileLists) Then
		...
		For I=0 To Ubound(FileArr)
		 Conn.Execute("Insert Into [KS_UploadFiles](ChannelID,InfoID,ClassID,FileName,IsAnnex,UserName,AddDate) values(" &ChannelID &"," & InfoID &"," & ChkClng(Session("UploadClassID")) & ",'" & FileArr(i) & "',0,'" & C("UserName") & "'," & SQLNowString&")")
		Next
	  End If
	  Session("UploadClassID")=0
	End Sub
	...
	Function GetFreight(ExpressID,ToCity,ByVal totalweight,ByRef ExpressCompany)
		 ...
		 If foundexpress=false Then
		  ...
		  RST.Open "select Top 1 a.sfweight,carriage,c_fee,w_fee,b.typename from KS_Delivery a inner join KS_DeliveryType b on A.ExpressID=B.TypeID where a.aExpressID="& expressid &" and (a.tocity='全国统一运费' or convert(varchar(200),tocity)='' or a.tocity is null)",conn,1,1
		  Else
		 ...
		 if (totalweight<=fweight) or (C_fee=0 and W_fee=0) then
		   ...
		   GetFreight=FormatNumber(carriage+(totalweight-fweight)*C_fee/W_fee,2,-1,-1)
		 end if
		End Function
	'**********************************************************************
	'函数名：ReturnSpecial
	...
	Function ShowPage(totalnumber, MaxPerPage, FileName, CurrPage,ShowTurn,PrintOut)
				 ...
				 Next
				 if CurrPage<>TotalPage Then pageStr=pageStr & "<a href=""" & FileName & "?page=" & TotalPage & ParamStr & """ class=""num"">..."& TotalPage &"</a>"
...
'*******************************************************************************************************************************
	...
	Public Function ReplaceUserFile(ReplaceContent,ChannelID)
		...
		re.Pattern = "(" &Setting(3)&Setting(91) &"user[^(""|'|\s)]*[.]{1}(gif|jpg|png|bmp|rar|doc|xsl|zip|exe))"
		Set BeyondFile = re.Execute(ReplaceContent)
		Set re = Nothing
		Dim Path,DateDir
		Path = GetUpFilesDir()
			...
			Dim PageStr, I, J, SelectStr
			 ...
			 Case 4
				 ...
				 If TotalPage>10 Then
					 If CurrPage=TotalPage Then
					 pageStr=pageStr & "<a href=""" & FileName & TotalPage & Extension &""" class=""curr"" style=""color:red"">..." & TotalPage & "</a>"
					 Else
  ...
  '======================================会员相关函数====================================
	...
	Public Function PointInOrOut(ChannelID,InfoID,UserName,InOrOutFlag,Point,User,Descript,ContributeFlag)
	  ...
	  If (Conn.Execute("Select top 1 * From KS_LogPoint Where UserName='" & UserName & "' and ChannelID=" & ChannelID & " and InfoID=" & InfoID & " And InOrOutFlag=" & InOrOutFlag).Eof) Or (ChannelID=0 And InfoID=0) or ContributeFlag=0 Then
		  ...
		  If IsObject(Session(SiteSN&"UserInfo")) Then Session(SiteSN&"UserInfo").DocumentElement.SelectSingleNode("row").SelectSingleNode("@point").Text=currPoint
	...
	Public Function ScoreInOrOut(UserName,InOrOutFlag,Score,User,Descript,ChannelID,InfoID)
	  ...
	  If (Conn.Execute("Select top 1 * From KS_LogScore Where UserName='" & UserName & "' and ChannelID=" & ChannelID & " and InfoID=" & InfoID & " And InOrOutFlag=" & InOrOutFlag).Eof) Or (ChannelID=0 And InfoID=0) Then
		  ...
		  If IsObject(Session(SiteSN&"UserInfo")) Then Session(SiteSN&"UserInfo").DocumentElement.SelectSingleNode("row").SelectSingleNode("@score").Text=currscore
		  Conn.Execute("Insert into KS_LogScore(UserName,InOrOutFlag,Score,CurrScore,[User],Descript,Adddate,IP,ChannelID,InfoID,[Times]) values('" & UserName & "',"& InOrOutFlag & "," & Score & ","&CurrScore & ",'" & replace(User,"'","""") & "','" & replace(Descript,"'","""") & "'," & SqlNowString & ",'" & replace(getip,"'","""") & "'," & ChannelID &"," & InfoID &",1)")
	  End If
	...
	Public Function MoneyInOrOut(UserName,ClientName,Money,MoneyType,InorOutFlag,PayTime,OrderID,Inputer,Remark,ChannelID,InfoID,MustIn)
	  ...
	  If (Conn.Execute("Select top 1 * From KS_LogMoney Where UserName='" & UserName & "' and ChannelID=" & ChannelID & " and InfoID=" & InfoID & " And IncomeOrPayOut=" & InOrOutFlag).Eof) Or (ChannelID=0 And InfoID=0) Or MustIn=1 Then
		  ...
		  If IsObject(Session(SiteSN&"UserInfo")) Then Session(SiteSN&"UserInfo").DocumentElement.SelectSingleNode("row").SelectSingleNode("@money").Text=currmoney
	      Conn.Execute("Insert into KS_LogMoney([UserName],[ClientName],[Money],[MoneyType],[IncomeOrPayOut],[OrderID],[Remark],[PayTime],[LogTime],[Inputer],[IP],[CurrMoney],[ChannelID],[InfoID],[Times]) values('" & UserName & "','" & ClientName & "'," & Money & "," & MoneyType & ","& InOrOutFlag & ",'" & OrderID & "','" & replace(Remark,"'","""") & "'," & SqlNowString & "," &SqlNowString & ",'" & replace(inputer,"'","""") & "','" & replace(getip,"'","""") & "'," & CurrMoney & "," & ChannelID & "," & InfoID & ",1)")
	  End If
