		  ...
		  '返回相应模型的自定义字段名称数组
		   Function Get_KS_D_F_P_Arr(ChannelID,Param)
		      Dim KS_RS_Obj:Set KS_RS_Obj=Server.CreateObject("ADODB.RECORDSET")
			   KS_RS_Obj.Open "Select FieldName,Title,Tips,FieldType,DefaultValue,Options,MustFillTF,Width,Height,FieldID,ShowOnUserForm,EditorType,ShowUnit,UnitOptions,ParentFieldName,MaxLength From KS_Field Where ChannelID=" & ChannelID &"  And ShowOnForm=1 " & Param & " Order By OrderID Asc",Conn,1,1
              ...
              If IsArray(F_Arr) Then
				For I=0 To Ubound(F_Arr,2)
				  If F_Arr(14,I)="0" Or KS.IsNul(F_Arr(14,I)) Then
					...
					End If
					Get_KS_D_F_I=Get_KS_D_F_I & " <td width=""85"" nowrap align=""right"" class='clefttitle'><strong>" & F_Arr(1,I) & "：</strong></td>" & vbcrlf
					Get_KS_D_F_I=Get_KS_D_F_I & " <td style=""word-break:break-all"">"
				   ...
				   Else
					   Select Case F_Arr(3,I)
						 Case 2
						   Get_KS_D_F_I=Get_KS_D_F_I & "<textarea style=""width:" & F_Arr(7,i) & "px;height:" & F_Arr(8,i) & "px"" rows=""5"" class=""upfile"" name=""" & F_Arr(0,i) & """>" & F_Value & "</textarea>"
						 Case 3,11
						 ...
						 Case 7
						   ...
						   Next
						 Case 10
							Get_KS_D_F_I=Get_KS_D_F_I & "<textarea id=""" & F_Arr(0,I) &""" name=""" & F_Arr(0,I) &""">"& Server.HTMLEncode(F_Value) &"</textarea><script type=""text/javascript"">CKEDITOR.replace('" & F_Arr(0,I) &"', {width:""" & F_Arr(7,i) &""",height:""" & F_Arr(8,i) & """,toolbar:""" & F_Arr(11,i) & """,filebrowserBrowseUrl :""Include/SelectPic.asp?from=ckeditor&Currpath="& KS.GetUpFilesDir() &""",filebrowserWindowWidth:650,filebrowserWindowHeight:290});</script>"
						 Case Else
						   Dim MaxLength:MaxLength=F_Arr(15,i)
						   If Not IsNumerIc(MaxLength) Or MaxLength="0" Then MaxLength=255
						   Get_KS_D_F_I=Get_KS_D_F_I & "<input maxlength=""" &MaxLength &""" type=""text"" class=""upfile"" style=""width:" & F_Arr(7,i) & """ name=""" & F_Arr(0,i) & """ id=""" & F_Arr(0,i) & """ value=""" & F_Value & """>"
					   End Select
				   End If
				   
				   If F_Arr(12,I)="1" and channelid<>101 Then 
		   ...
		   '检查录入的自定义字段
		   Sub CheckDiyField(channelid,byref UserDefineFieldArr,byref ErrMsg)
		      Dim I
			  UserDefineFieldArr=Get_KS_D_F_Arr(ChannelID)
		     If IsArray(UserDefineFieldArr) Then
				For I=0 To Ubound(UserDefineFieldArr,2)
				 If Cint(UserDefineFieldArr(6,I))=1 And KS.IsNul(KS.G(UserDefineFieldArr(0,I))) Then ErrMsg = ErrMsg & UserDefineFieldArr(1,I) & "必须填写!\n"
				 If (Cint(UserDefineFieldArr(3,I))=4 or Cint(UserDefineFieldArr(3,I))=12) And Not KS.IsNul(KS.G(UserDefineFieldArr(0,I))) And Not Isnumeric(KS.G(UserDefineFieldArr(0,I))) Then ErrMsg = ErrMsg& UserDefineFieldArr(1,I) & "必须填写数字!\n"
				 If Cint(UserDefineFieldArr(3,I))=5 And Not KS.IsNul(KS.G(UserDefineFieldArr(0,I))) And Not IsDate(KS.G(UserDefineFieldArr(0,I))) Then ErrMsg = ErrMsg& UserDefineFieldArr(1,I) & "必须填写正确的日期!\n" 
				 If Cint(UserDefineFieldArr(3,I))=8 And Not KS.IsValidEmail(KS.G(UserDefineFieldArr(0,I))) and Cint(UserDefineFieldArr(6,I))=1 Then ErrMsg = ErrMsg& UserDefineFieldArr(1,I) & "必须填写正确的Email格式!\n" 
...
'exit sub
				 ...
				 Dim SearchJS,FsoPath
				  ...
				  SearchJS = SearchJS & "        <select name=""ClassID"" style=""width:150px"">" & vbCrLf
