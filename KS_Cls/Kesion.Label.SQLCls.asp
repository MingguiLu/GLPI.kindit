		  ...
		  If not IsObject(Application(KS.SiteSN&"_sqllabellist")) Then
			 ...
			 RS.Close:Set Rs=Nothing
				...
				For i=0 to objNode.ChildNodes.length-1 
					...
					Str=Replace(Replace(Replace(Replace(Replace(objAtr.Attributes.item(2).Text,"{$Field","{#Field"),"{$AutoID}","{#AutoID}"),"{$IF","{#IF"),"{$Param","{#Param"),"{$GetItemUrl}","{#GetItemUrl}")  '避免Field字段被替换掉,先转为#
	 ...
	 Function ReplaceCondition(byval str)
	  ...
	  Dim FieldParam,FieldParamArr,ReturnFieldValue,I
		...
		Function ReplaceDIYFunctionLabel(SqlLabel,GetFrom)
		  Dim I,KS_RS_Obj,UserParamArr,FunctionLabelParamArr,CirLabelContent,FunctionSQL,LabelContent
