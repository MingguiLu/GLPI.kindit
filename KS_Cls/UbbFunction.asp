...
Public Function Ubbcode(strcontent,n)
	...
	if re.Test(strcontent) then strcontent=re.replace(strcontent,"<a onfocus=""this.blur()"" href=""$1"" target=new><img src=""$1"" border=""0"" alt=""按此在新窗口浏览图片"" onload=""javascript:if(this.width>screen.width-333)this.width=screen.width-333""></a>")

	re.pattern="\[img=*([0-9]*),*([0-9]*)\](.*?)\[\/img\]"
	if re.Test(strcontent) then strcontent=re.replace(strcontent,"<a onfocus=""this.blur()"" href=""$3"" target=new><img src=""$3"" border=""0""  width=""$1"" heigh=""$2"" alt=""按此在新窗口浏览图片"" onload=""javascript:if(this.width>screen.width-333)this.width=screen.width-333""></a>")
	...
	if re.Test(strcontent) then strcontent= re.replace(strcontent,"<a target=""new"" href=""tencent://message/?uin=$1&Site=" & KS.Setting(0) &"&Menu=yes""><img border=""0"" src=""http://wpa.qq.com/pa?p=4:$1:4"" alt=""点击这里给我发消息""></a>")
    '水平线
	re.pattern="\[hr\]"
