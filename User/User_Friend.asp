       ...
       Public Sub loadMain()
		...
		<div class="tabs">	
			<ul>
				...
				<li<%If action="online" then KS.Echo " class='select'"%>><a href="user_friend.asp?action=online">在线用户</a></li>
	  ...
	  End Sub
		...
		<form action="?action=addsave" method="post" name="myform">
			<tr height="23" class="title">
				<td width="25%" height="25" align="center">添加好友<span style='font-weight:normal'>(通过对方验证才能成为好友)</span></td>
			</tr>
			<tr> 
			  <td>
		...
		Sub mailsave()
		 ...
		 for i=0 to ubound(email)
		   ...
		   content=content & "<td><p>Hi,最近又在做些什么活动,我发现一个叫做" & KS.Setting(1) & "的网站,这里有很多好玩的东西,在这里我发现了好多相识的老朋友和刚认识的新朋友,我们都有共同的兴趣爱好,你也一起加入吧!</p><p>点击这个链接，把我加为好友吧！<br /><a href=""" & KS.GetDomain & "?do=reg&uid=" & server.urlencode(KSUser.UserName) & "&f=r&u=" & server.urlencode(email(i)) &""" target=""_blank"">" & KS.GetDomain & "?do=reg&uid=" & server.urlencode(KSUser.UserName) & "&f=r&u=" & server.urlencode(email(i)) &"</a></p>"
		   content=content & "注明：本邮件是您的好友<a href=""" & KS.GetDomain & "space?" & KSUser.UserName & """ target=""_blank"">" & KSUser.GetUserInfo("RealName") & "</a>通过" & KS.Setting(1) & "发送，无需回复。"
...
<Select id="birth_d" name="birth_d" style="width:50px"> 
  ...
  %>
		...
		MaxPerPage=12
		Response.Write "<table width=""98%"" class='border' align=center cellpadding=2 cellspacing=1 border=0><tr class=title><td height=""30"">&nbsp;帮您推荐<span style='font-weight:normal'>(以下是在本站最活跃会员)</span></td></tr></table>"
		Response.Write "<table width=""98%"" class='border' align=center cellpadding=2 cellspacing=1 border=0><tr class='tdbg'>"
		...
		if not (rs.bof and rs.eof) then
			...
			user_face=rs("userface")
								If user_face="" or isnull(user_face) then 
								if rs("sex")="男" then  user_face="images/face/boy.gif" else user_face="images/face/girl.gif"
							    End If
			If lcase(left(user_face,4))<>"http" then user_face=KS.Setting(2) & "/" & user_face
				...
				Response.Write "<div class=""mem_left_photo""><img width=""62"" height=""58"" src=""" & user_face&"""></div></td>"
