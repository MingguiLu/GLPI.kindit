   ...
   Do While Not RS.Eof
		%>
                <tr class='tdbg' onMouseOver="this.className='tdbgmouseover'" onMouseOut="this.className='tdbg'">
                       <td height='30' class="splittd"><a href="User_Order.asp?Action=ShowOrder&ID=<%=RS("ID")%>"><%=rs("orderid")%></a></td>
...
If RS("Status")=3 Then
  ...
  <table class=border cellSpacing=1 cellPadding=2 width="98%" align="center" border=0>
    ...
    <tr class=tdbg>
      ...
      <td align=left><%=formatnumber(KSUser.GetUserInfo("Money"),2,-1)%> 元 <%if Round(KSUser.GetUserInfo("Money"))<=0 then response.write "<a href=""user_payonline.asp"">资金不足,请点此充值</a>"%></td>
    </tr>
    <tr class=tdbg>
  ...
  <Input id=ID type=hidden value="<%=rs("id")%>" name="ID"> 
		...
		Sub SavePayment()
		 ...
		 End If
		 If Round(Money)>Round(KSUser.GetUserInfo("Money")) or Round(KSUser.GetUserInfo("Money"))<=0  Then
		  %>
		  <br><br>
...
OrderDetailStr=OrderDetailStr & "						  <td colspan='2' width='38%'>快递公司：" 
	 ...
	 If foundexpress=false Then
	  ...
	  RST.Open "select Top 1 a.fweight,carriage,c_fee,w_fee,b.typename from KS_Deliverys a inner join KS_DeliveryType b on A.ExpressID=B.TypeID where a.aExpressID="& rs("delivertype") &" and (convert(varchar(200),tocity)='' or a.tocity is null)",conn,1,1
	  Else
