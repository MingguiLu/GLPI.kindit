       ...
       Public Sub loadMain()
		...
		<div class="tabs">	
		   <ul>
			 ...
			 <div style="padding-left:20px;"><img src="images/fav.gif" align="absmiddle"><a href="User_Photo.asp?Action=Add"><span style="font-size:14px;color:#ff3300">上传照片</span></a>
...
<tr class="tdbg">
			...
			<tr class="tdbg"> 
                  ...
                  <td><select style="width:160px" onChange="if(this.options[selectedIndex].value=='3'){document.myform.all.mmtt.style.display='block';}else{document.myform.all.mmtt.style.display='none';}"  name="flag">
                    ...
                    </select><span class="msgtips">可以设置为只有权限的用户才能浏览。 </span><span class=child id=mmtt name="mmtt" <%if flag<>3 then%>style="display:none;"<%end if%>>密码：<input type="password" name="password" style="width:160px" maxlength="16" value="<%=password%>" size="20"></span> 
				   </td>
            </tr>
            <tr class="tdbg">
              <td class="clefttitle">相册封面：</td>
              <td><input class="textbox" name="PhotoUrl" type="text" id="PhotoUrl" style="width:230px; " value="<%=PhotoUrl%>" />                  <span class="msgtips">只支持jpg、gif、png，小于50k，默认尺寸为85*100</span>
            ...
            <tr class="tdbg">
              ...
              <td><textarea class="textarea" name="Descript" id="Descript" cols=50 rows=6><%=Descript%></textarea>              <span class="msgtips">关于此相册的简要文字说明。</span>
				  </td>
            </tr>
            <tr class="tdbg">
			  <td></td>
					   ...
					   <select class="textbox" size='1' name='XCID' style="width:150">
						 ...
						 <div id="pub1" style="display:none;margin-top:5px">
						   ...
						   <tr>
							 ...
							 <td><select onChange="if(this.options[selectedIndex].value=='3'){document.myform.all.mmtt.style.display='block';}else{document.myform.all.mmtt.style.display='none';}"  name="flag"><option value="1" selected>完全公开</option>
	 ...
	 <script type="text/javascript">
			...
			window.onload = function () {
				swfu = new SWFUpload({
					...
					button_width: 195,
					button_height: 22,
					button_text : '<span class="button">批量上传(单图限制2 MB)</span>',
  ...
  Sub AddSave()
    Dim RSObj,Descript,PhotoUrlArr,i,UpFiles,PhotoUrl,PubType,ClassID
				...
				If PubType=1 Then
			        RSObj.Open "Select top 1 * From KS_Photoxc" ,conn,1,3
					  ...
					  XCID=RSObj("id")
					   ...
					   Call KSUser.AddLog(KSUser.UserName,"创建了相册!名称: "& Title & " <a href=""../space/?" & KSUser.GetUserInfo("userid") & "/showalbum/" & XCID & """ target=""_blank"">查看</a>",104)
				RSObj.Close
				End If
				...
				RSObj.Open "Select top 1 * From KS_PhotoZP",Conn,1,3
				 ...
				 Call KSUser.AddLog(KSUser.UserName,"上传了" & Ubound(PhotoUrlArr)+1 & "张照片到相册! <a href=""../space/?" & KSUser.GetUserInfo("userid") & "/showalbum/" & xcid & """ target=""_blank"">查看</a>",104)
