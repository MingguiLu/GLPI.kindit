...
Class MyComment
		...
		<div class="tabs">						  
			<ul>
				<li<%If flag=0 then response.write " class='select'"%>><a href="User_MyComment.asp">我参与的评论(<span class="red"><%=Conn.Execute("Select count(id) from KS_Comment" & Param & "")(0)%></span>)</a></li>
		...
		<div class="tabs">						  
			<ul>
				<li<%If flag=0 then response.write " class='select'"%>><a href="User_MyComment.asp?action=My">用户的评论(<span class="red"><%=Conn.Execute("Select count(id) from KS_Comment" & Param & "")(0)%></span>)</a></li>
			</ul>
          </div>
			<%end if%>
