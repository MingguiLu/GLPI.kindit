     ...
     %>                     
	   ...
	   Sub exchange()
		...
		If RS.Eof And RS.Bof Then
		  ...
		  Exit Sub
		End If
		IF KS.ChkClng(KSUser.GetUserInfo("Score"))< KS.ChkClng(RS("Score")) Then
		...
		<table width="98%" border="0" align="center" cellpadding="0" cellspacing="1">
			<tr>
			  <td style="font-size:14px">
			  		亲爱的<font color=red><%=KSUser.UserName%></font>用户!您当前可用积分:<font color=red><%=KSUser.GetUserInfo("Score")%></font>分,最多可兑换<font color=blue><%=Cint(KSUser.GetUserInfo("Score")/RS("Score"))%></font>件,如果您确定兑换本礼品,请认真填写以下收货信息,兑换订单一旦提交,便不可取消!              </td>
			</tr>
			<tr>
			...
			<tr>
			    <td  class="splittd" height="35">
				   ...
				   <select name="amount">
				   <%dim k,endnum
				   endnum=Cint(KSUser.GetUserInfo("Score")/RS("Score"))
	   ...
	   Sub exchangesave()
		...
		If RS.Eof And RS.Bof Then
		  ...
		  Exit Sub
		End If
		IF KSUser.GetUserInfo("Score")< RS("Score") Then
		  Call KS.AlertHistory("对不起,您的积分不足!",-1)
		  Exit Sub
		ElseIf KS.ChkClng(RS("Quantity"))<=0 Then
		  Call KS.AlertHistory("对不起,该礼品已兑换完毕!",-1)
