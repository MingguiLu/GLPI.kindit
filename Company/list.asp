...
Class SiteIndex
		...
		Sub getadslist()
		 ...
		 If DataBaseType=0 then
		  ...
		  Param=Param & " and datediff(day,addDate," &SqlNowString & ")<datatimed"
		 end if
		 rs.cursorlocation = 3
		...
		Sub GetClassList()
		 ...
		 if Pid<>0 then
			 ...
			 for i=1 to 5
			   ...
			   str=str & "<div style=""height:20px;""><a href=""list.asp?id=" & rs(0) & """>" & rs(1) &"</a>(" & conn.execute("select count(id) from ks_enterprise where status=1 and smallclassid=" & rs(0))(0) &") </div>" & vbcrlf
		...
		Sub GetCompanyList()
		 ...
		 Dim SqlStr:SqlStr="select b.logo,[Domain],a.username,a.companyname,a.province,a.city,a.recommend,a.intro,a.RegisteredCapital,a.address,a.telphone,a.viptf,a.placevalue,b.userid from ks_enterprise a inner join ks_blog b on a.username=b.username" & Param & SortStr
		...
		Sub ShowContent(RS)
		 ...
		 Dim I,logo,n,url,msgUrl
 		 c_str="<div class=""productorder""><a href='?pid=" & Pid & "&id="&ks.g("id") & "'>默认排序</a> <a href='?recommend=1&pid=" & Pid & "&id="&ks.g("id") & "'>推荐企业</a> <a href='?t=1&pid=" & Pid & "&id="&ks.g("id") & "'>加盟时间</a></div>"
		...
		Sub getarealist()
		  ...
		  If IsArray(SQL) Then
			  ...
			  For i=0 To Ubound(SQL,2)
				...
				astr=astr & "<td><img src='../images/arrow_r.gif'> <a href=""list.asp?province=" & server.URLEncode(province) & "&provinceid=" & ks.s("provinceid") & "&city=" & server.URLEncode(sql(0,n)) & """>" & sql(0,n) & "</a></td>"
				 Else
				astr=astr & "<td><img src='../images/arrow_r.gif'> <a href=""list.asp?province=" & server.URLEncode(sql(1,n)) & "&provinceid=" & SQL(0,n) & """>" & sql(1,n) & "</a></td>"
				 End If
				n=n+1
