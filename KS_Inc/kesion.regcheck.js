...
function out_username(){
	...
	$.ajax({type:"get",async:false,url:"user/reg/regajax.asp?action=checkusername&username="+escape(document.getElementById("UserName").value)+"&anticache=" + Math.floor(Math.random()*1000),cache:false,dataType:"html",success:function(d){
	     var s=unescape(d);
		 ajaxchk=s.split('|')[0];
		 ajaxstr=s.split('|')[1];
	}
...
function out_email(){
	...
	$.get("user/reg/regajax.asp",{action:"checkemail",email:escape(str)},function(d){
	     var s=unescape(d);
		 ajaxchk=s.split('|')[0];
		 ajaxstr=s.split('|')[1];
		if (ajaxstr!=null){
...
{	var obj=document.getElementById("chkcodemsg");
	...
	$.get("user/reg/regajax.asp",{action:"checkcode",code:escape(document.getElementById("Verifycode").value)},function(d){
	     var s=unescape(d);
		 ajaxchk=s.split('|')[0];
		 ajaxstr=s.split('|')[1];
	})
