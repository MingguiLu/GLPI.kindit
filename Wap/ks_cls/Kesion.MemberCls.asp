...
Class UserCls
			Private KS,I  
			'---------定义会员全局变量开始---------------
			Public ID,GroupID,UserName,PassWord,Question,Answer,Email,Node
		   ...
		   End Sub	
			...
			Sub AddLog(username,note,ico)
			  Conn.Execute("Insert Into KS_UserLog([userid],[username],[note],[adddate],[ico]) values(" & GetUserInfo("userid") &",'" & UserName & "','" & replace(note,"'","""") & "'," & SqlNowString & "," & ico & ")")
			End Sub
'删除模型信息数据
