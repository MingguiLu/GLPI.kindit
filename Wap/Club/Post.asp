...
Class GuestPost
		...
		Sub EditPost()
			...
			Dim RSC:Set RSC=Conn.Execute("Select top 1 Content From " & PostTable & " Where ParentID=0 and topicid=" & id)
			If Not RSC.Eof Then
			  Content=RSC("Content")
			Else 
			  Content=" "
			...
			Response.Write "内容:<input name=""Memo" & Minute(Now) & Second(Now) & """ type=""text"" value=""" & Server.HTMLEncode(Content) & """ emptyok=""false""/><br/>" &vbcrlf
		...
		Sub SaveEditPost()	
			...
			If A = True Then 
			   ...
			   RSObj.Update
			   RSObj.Close
			   RSObj.Open "Select top 1 * From " & postTable & " Where TopicID=" & ID & " and parentid=0",conn,1,3
		...
		Sub SaveConnectPost()
		    Dim RS:Set RS=Conn.Execute("Select top 1 postTable From KS_GuestBook Where ID="&ID&"")
			If RS.EOF Then
			   Response.write "非法参数!<br/>"
			   Prev=True
			   Exit Sub
			End If
			Dim Memo,SplitStrArr,I,postTable
			...
			End If
			Dim RSObj:Set RSObj=KS.InitialObject("Adodb.Recordset")
			RSObj.Open "Select top 1 Content From " & postTable & " Where parentid=0 and topicID=" & ID,Conn,1,3
			RSObj(0)=RsObj(0)&KS.HTMLEncode(Memo)
	   ...
	   End Function
		...
		Sub SaveData()
			...
			Doc.load(GetMapPath&"/Config/clubtable.xml")
			...
			RSObj.AddNew 
			RSObj("UserName") = KS.HTMLEncode(Name)
			RSObj("UserID")=KS.ChkClng(KSUser.GetUserInfo("userid"))
			RSObj("Email") = ""
			RSObj("HomePage") = "http://www.kesion.com"
			RSObj("Face") = "1.gif"
			RSObj("IsPic")=0
			RSObj("TxtHead") = "01.gif"
			RSObj("Subject") = KS.HTMLEncode(Subject)
			RSObj("Oicq") = ""
			RSObj("GuestIP") = IP  
			If KS.ChkClng(KS.Setting(52))=1 Then  
			RSObj("Verific")=0
			Else
			RSObj("Verific")=1
			End If
			...
			RSObj("TotalReplay")=0
			RSObj("LastReplayUser")=KS.HTMLEncode(Name)
			RSObj("LastReplayUserID")=KS.ChkClng(KSUser.GetUserInfo("userid"))
			RSObj("PostTable")=TableName
			RSObj.Update
			RSObj.MoveLast
			TopicID=RSObj("ID")
			N_LastPost=RSObj("ID")&"$"& now & "$" & Replace(left(subject,200),"$","") & "$" & KS.ChkClng(KSUser.GetUserInfo("userid")) &"$$$"
			RSObj.Close
			...
			If KS.ChkClng(BSetting(3))>0 and LoginTF=true Then
				 Call KS.ScoreInOrOut(KSUser.UserName,1,KS.ChkClng(BSetting(3)),"系统","在论坛发表主题[" & Subject & "]所得!")
			End If
