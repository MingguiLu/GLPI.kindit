...
Class Display
        Private KS,ID
		Private RST,Master,BoardName,PostTable,IsBest
		...
		Sub GetSubJect()
			...
			BoardName=LFCls.GetSingleFieldValue("select top 1 BoardName from KS_GuestBoard where ID=" & KS.ChkClng(FCls.RefreshFolderID))
			Response.Write "<wml>" &vbcrlf
			...
			Response.Write "作者:<a href=""../User/ShowUser.asp?Keyword="&RST("UserName")&"&amp;" & KS.WapValue & """>" & KS.GetUserRealName(Rst("UserName")) & "</a> ("&KS.DateFormat(RST("AddTime"),17)&")"
			Response.Write " 已有" & RST("hits") & "人关注过本帖 共" & RST("TotalReplay") & "人跟帖<br/>"
		...
		Sub GetReplayList()
		    MaxPerPage=10
			SqlStr = "SELECT * From " & PostTable & " where TopicID=" & KS.ChkClng(KS.S("ID")) & " ORDER BY ID" 
		...
		Sub GetTopicList(RS)
            ...
            End If
		 	Do While Not RS.EOF
			   ...
			   If N=1 Then
			   Response.Write "楼主:"
			   If RS("UserName")=KSUser.UserName or Check=True Then
			   'Response.Write "<a href=""DownUrls.asp?ID=" & ID &"&amp;" & KS.WapValue & """ >增加下载附件</a> "
			   Response.Write "<a href=""Post.asp?Action=EditPost&amp;ID=" & ID &"&amp;" & KS.WapValue & """>编辑</a> "
			   Response.Write "<a href=""Post.asp?Action=ConnectPost&amp;ID=" & ID &"&amp;" & KS.WapValue & """ >续写</a> "
			   Response.Write "<a href=""Display.asp?ID=" & ID & "&amp;Action=DelSubject&amp;" & KS.WapValue & """>删除</a><br/> "
			   End If
		...
		Sub GetWriteComment()
			...
			Else
			   ...
			   Response.Write "<postfield name=""posttable"" value=""" & PostTable & """/>"
		...
		Sub DelSubject()
			...
			Conn.Execute("delete from "& getPostTable(id) &" where TopicID=" & ID)
		...
		Sub DelReply()
		    If Cbool(Check)=False Then
			   Call KS.ShowError("系统提示！","对不起，你没有设置的权限!")
			End If
			Conn.Execute("delete from "& getPostTable(id) &" where ID=" & KS.ChkClng(KS.S("ReplyID")))
		...
		Sub Verify()
		    If Cbool(Check)=False Then
			   Call KS.ShowError("系统提示！","对不起，你没有设置的权限!")
			End If
			Conn.Execute("Update "& getPostTable(id) &" set verific=1 where ID=" & KS.ChkClng(KS.S("ReplyID")))
