...
Class ReplyCls
	   ...
	   End Sub
		...
		function getPostTable()
		   dim rs :set rs=conn.execute("select top 1 PostTable From KS_GuestBook Where ID=" & KS.ChkClng(KS.S("TopicID")))
		   If RS.Eof Then
	   ...
	   Sub WriteReply()
		   ...
		   Response.Write "<postfield name=""postTable"" value=""" & getPostTable & """/>" &vbcrlf
	   ...
	   Sub LeadReply()
		   ...
		   Dim PostTable:PostTable=getPostTable
		   Dim SqlStr:SqlStr = "SELECT top 1 * From " & PostTable &" where TopicID=" & TopicID & " And ID=" & LeadID & "" 
		   ...
		   Else
			  ...
			  Response.Write "<postfield name=""Page"" value=""" & KS.S("Page") & """/>" &vbcrlf
			  Response.Write "<postfield name=""PostTable"" value=""" & PostTable & """/>" &vbcrlf
			  Response.Write "</go></anchor>" &vbcrlf
			  Response.Write "<br/>" &vbcrlf
		   End If
       End Sub
	   ...
	   Sub SaveData()
		   ...
		   Dim PostTable:PostTable=KS.S("posttable")
		   ...
		   Dim Subject:Subject=Conn.Execute("Select top 1 subject From KS_GuestBook Where ID=" & TopicID)(0)
			...
			N_LastPost=topicid & "$" & now & "$" & Replace(Subject,"$","") &"$" & KS.ChkClng(KSUser.GetUserInfo("userid")) &"$$$"
			
			If KS.ChkClng(BSetting(4))>0 and LoginTF=true Then
				 Call KS.ScoreInOrOut(KSUser.UserName,1,KS.ChkClng(BSetting(4)),"系统","在论坛回复主题[" & Subject & "]所得!")
			End If
