...
Class SiteIndex
		...
		Public Sub Kesion()
			...
			FileContent = Replace(FileContent,"{$TopicNum}",doc.documentElement.attributes.getNamedItem("topicnum").text)'主题
			   ...
			   For Each Pnode In Xml.DocumentElement.SelectNodes("row[@" & BParam & "]")
				   ...
				   For Each Node In Xml.DocumentElement.SelectNodes("row[@parentid=" & Pnode.SelectSingleNode("@id").text & "]")
					 ...
					 Else
					  ...
					  str=str & "最新帖:<a href=""display.asp?id=" & Replace(LastPost_A(0),"{","｛") & "&amp;" & KS.WapValue & """>" & Replace(LastPost_A(2),"{","｛")& "</a> 总主题:" & Node.SelectSingleNode("@topicnum").text & "帖 总帖子:" & Node.SelectSingleNode("@postnum").text & "帖 今日帖:" & Node.SelectSingleNode("@todaynum").text & "帖<br/><br/>"
					  end if
					 End If
					 LC=LC&str
				  Next
