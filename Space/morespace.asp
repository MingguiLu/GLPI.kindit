 ...
 if ks.s("key")<>"" then param=param & " and blogname like '%" & ks.r(ks.s("key")) &"%'"
	...
	Dim RSObj:Set RSObj=Server.CreateObject("ADODB.RECORDSET")
		rsobj.open SQLStr ,conn,1,1
		         If RSObj.EOF and RSObj.Bof  Then
				 	str=str & "<tr><td style=""border: #efefef 1px dotted;text-align:center"" colspan=4><p>对不起，没有找到空间! </p></td></tr>"
				 Else
  ...
  Sub ShowSpaceList(rs)
   ...
   do while not rs.eof
	 ...
	 str=str & "<tr>"
      str=str & "<td class=""mysplittd"" style=""width:40px""><img title=""创建时间：" & rs("adddate") & """ style=""border:1px solid #efefef;padding:2px"" src=""" & Logo & """ width=""52"" height=""52"" /></td><td class=""mysplittd"">"
	  ...
	  If Not RSS.Eof Then
	   ...
	   If RSS("Istalk")="1" Then
	    str=str& "<a href='" & spacedomain & "/log/" & rss("id") & "' target='_blank'>[博文]</a>"
	   Else
	    str=str& "<a href='" & spacedomain & "/fresh' target='_blank'>[新鲜事]</a>"
	   End If
	    str=str &"- <a href='" & spacedomain & "/log/" & rss("id") & "' target='_blank'>评论(<font color=red>" & rss("totalput") & "</font>)</a></div>"
	  End If
	  RSS.Close
