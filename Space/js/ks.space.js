...
function freshBlur(obj){
	if ($("#"+obj).val()==''){
	  $("#"+obj).val('随便说点什么，让好友们知道你的心情、在做什么……（最少10个字）');
	 }
	  $("#"+obj).attr("style","");
   if ($("#"+obj).val()!='随便说点什么，让好友们知道你的心情、在做什么……（最少10个字）'){
...
{  
		...
		{ 
			...
			if ($('#Content').val()==''||$('#Content').val()=='既然来了，就顺便留句话儿吧...')
			{
			 alert('请输入留言内容!');
			 return false;
			}
		...
		function CheckPostMsg(){
			...
			ksblog.ajaxFormSubmit(document.myform,'PostMsgBack')
		}
		function PostMsgBack(){
			alert('恭喜，您的留言已提交！');
			location.reload();
		}
		function ShowLogin()
