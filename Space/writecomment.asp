  ...
  case else
    Response.Write("document.write('" & GetWriteComment(KS.ChkClng(KS.S("UserID")),KS.S("ID"),KS.S("Title"),KS.S("UserName")) & "');")
		...
		If KS.SSetting(25)="0" And KS.IsNul(KS.C("UserName")) Then
		  GetWriteComment="<div style=""margin:20px""><strong>温馨提示：</strong>只有会员才可以发表评论,如果是会员请先<a href=""javascript:ShowLogin()"">登录</a>,不是会员请点此<a href=""../?do=reg"" target=""_blank"">注册</a>。</div>"
		Else
		...
		GetWriteComment = GetWriteComment & " style=""color:#999;width:35%;border:1px solid #ccc;background:#FBFBFB;""/><br/>主页："
		GetWriteComment = GetWriteComment & "    <input name=""HomePage"" maxlength=""150"" value=""" & HomePage & """ type=""text"" id=""HomePage"" style=""color:#999;width:55%;border:1px solid #ccc;background:#FBFBFB;"" /><br/>标题："
        ...
        Sub CommentSave()
			...
			RS.UpDate
			 ...
			 If KS.C("UserName")<>"" and  KS.S("From")<>"1" Then
			  Call KSUser.AddLog(KS.C("UserName"),"对<a href=""{$GetSiteUrl}space/?" & KS.S("UserID") &""" target=""_blank"">" & KS.S("UserName") & "</a>写的博文[<a href=""{$GetSiteUrl}space/?" & KS.S("UserID") & "/log/" & ID & """ target=""_blank"">" & KS.S("OriTitle") & "</a>]发表了评论!",100)
			 End If
