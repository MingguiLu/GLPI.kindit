...
Class Admin_Form
		...
		Sub GetCode()
		  ...
		  Do While Not RS.Eof
			...
			If TypeFlag=1 Then
			.echo "<textarea style='width:500px;height:50px' name='s" & rs(0) & "'><div id=""DocRating"">打分系统加载中...</div><script src=""{$GetSiteUrl}ks_inc/jquery.js""></script>" & vbcrlf
			.echo "<script src=""{$GetSiteUrl}ks_inc/kesion.box.js""></script>" & vbcrlf & "&lt;script type=&quot;text/javascript&quot;&gt;" &vbcrlf
			.echo"var url='{$GetSiteUrl}'; " &vbcrlf & "var title='{$FL_Title}';" &vbcrlf & "var channelid={$ChannelID};  " &vbcrlf & "var infoid={$InfoID}; " &vbcrlf & "var itemid=" & RS(0) &";" &vbcrlf
			.echo "&lt;/script&gt;" & vbcrlf
			.echo "<script src=""{$GetSiteUrl}ks_inc/docrating.js""></script>"
			.echo "</textarea>"
			Else
