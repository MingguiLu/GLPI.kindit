...
Class Admin_Field
		...
		Private FieldName,ID,Contact, Title, Tips, FieldType, DefaultValue, MustFillTF, ShowOnForm, ShowOnUserForm,Options,OrderID,AllowFileExt,MaxFileSize,Width,Height,EditorType,ShowUnit,UnitOptions,ParentFieldName,MaxLength
		...
		.Write "<body leftmargin='0' topmargin='0' marginwidth='0' marginheight='0'>"
		 ...
		 If Optype = "Edit" Then
			 ...
			 End If
	  Else
	     FieldName="KS_":FieldType=1:MustFillTF=0:ShowOnForm=1:ShowOnUserForm=1:AllowFileExt="jpg|gif|png":MaxFileSize=1024:Width=200:Height=80:EditorType="Basic":ShowUnit=0:MaxLength=255
	  ...
	  End If
		...
		.Write  "{if (s==3||s==6||s==7||s==11){ $('#OptionsArea').show();} else $('#OptionsArea').hide();if (s==7)$('#darea').show();else $('#darea').hide();if(s==9)$('#extarea').show();else $('#extarea').hide(); if(s==10)$('#editorarea').show();else $('#editorarea').hide();if (s==2||s==10) $('#heightarea').show();else $('#heightarea').hide();if(s==11) $('#ldArea').show(); else $('#ldArea').hide();}"
