	...
	Public Sub Kesion()
	 ...
	 Sub ItemInfoMain()
		...
		function CreateHtml(){  
		   var ids=get_Ids(document.myform);
			if (ids!='')
		new KesionPopup().PopupCenterIframe('发布选中文档','Include/RefreshHtmlSave.Asp?ChannelID=<%=ChannelID%>&Types=Content&RefreshFlag=IDS&ID='+ids,530,110,'no')
			else 
			alert('请选择要发布的文档!');
		}	
		function MoveClass(){
		   var ids=get_Ids(document.myform);
			if (ids!='')
		new KesionPopup().PopupCenterIframe('批量移动选中文档','KS.ItemInfo.asp?ChannelID=<%=ChannelID%>&action=MoveClass&ID='+ids,530,110,'no')
			else 
			alert('请选择要移动的文档!');
		}	
		function CreateNews(){   
		...
		End If
		 ...
		 .echo (" - <a href='?ChannelID=" & ChannelID & "&status=11&ComeFrom=" & ComeFrom & "'><font color=#999999>视频</font></a>")
		 ...
		 End If
		  ...
		  If ChannelID=5 Then
		   ...
		   OrderArray(t+3)="市场价↓|Price_Market|1": OrderArray(t+4)="市场价↑|Price_Market|0"
		   OrderArray(t+5)="当前零售价↓|Price|1": OrderArray(t+6)="当前零售价↑|Price|0"
		   OrderArray(t+7)="原始零售价↓|Price_Original|1" :OrderArray(t+8)="原始零售价↑|Price_Original|0"
		   OrderArray(t+9)="会员价↓|Price_Member|1" :OrderArray(t+10)="会员价↑|Price_Member|0"
		...
		.echo ("<div style=""width:100%;padding-bottom:15px;overflow-x: auto; height:auto"">")
		 ...
		 .echo ("<td width='35' align='center' nowrap>选择</td>")
			 ...
			 End If
		.echo ("<td height=15 nowrap>标题</td>")
	   ...
	   End If
		...
		Else
		  ...
		  End If
			  ...
			  .echo ("<td align='center' width='100'><input class='button' onclick='CreateHtml()' type='button' value='发布'> <input class='button' onclick='MoveClass()' type='button' value='移动'></td>")
			  End If
			  .echo ("<td>")
					...
					End If
						...
						.echo ("<td class='splittd' align=center><input name='id'  onclick=""chk_iddiv('" & ItemId & "')"" type='checkbox' id='c"& ItemId & "' value='" &ItemId & "'></td>")
						 ...
						 End If
							 ...
							 If ChannelID=5 Then
							  if INode.SelectSingleNode("@producttype").text="2" Then .echo "<span title='涨价出售' style='color:green'> ↑</span>" Else If INode.SelectSingleNode("@producttype").text="3" Then .echo "<span style='color:red' title='降价出售'> ↓</span>"
							 End If
							 .echo ( "</span></span></td>")						
					...
					End If	 
							 ...
							 ElseIf ComeFrom="Verify" Then
							  ...
							  Else
								...
								End If
							 Else
							 .echo (" <a href='" & ItemManageUrl & "?Page=" & Page & "&Action=Edit&" &SearchParam&"&ID=" & ItemId & "' onclick='parent.frames[""BottomFrame""].location.href=""KS.Split.asp?ChannelID=" & ChannelID &"&ComeFrom="&ComeFrom&"&OpStr="&Server.URLEncode("编辑" & KS.C_S(ChannelID,3)) & "&ButtonSymbol=AddInfo&FolderID=" & ID & """;'>修改</a> | <a href=""?Action=Recely&Page=" & Page & "&" & SearchParam & "&ID=" & ItemId & """ onclick=""return (confirm('确定将该" & KS.C_S(ChannelID,3) & "放入回收站吗?'))"">回收站</a>")
							 If ChannelID=5 Then
							 .echo (" | <a href='KS.ProImages.asp?ProID=" &ItemId &"&ChannelID=5'>图片</a>")
							 End If
							 End If
							 .echo ("</td>")
							 .echo ("</tr>")
