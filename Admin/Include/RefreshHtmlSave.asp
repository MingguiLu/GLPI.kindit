		...
		If NowNum = "" Then NowNum = 0
		  Select Case RefreshFlag
		   ...
		   Case "IDS"
				...
				UpdateSql="Update "& Table & " Set RefreshTF=1 "  &R_SQL&" and ID IN(" & ID & ") Order By ID"
		   ...
		   Case "Date"
			 ...
			 If CInt(DataBaseType) = 1 Then         'Sql
				 RefreshTotalNum=conn.execute("select count(ids) from " & Table  & R_Sql & " And AddDate>= '" & StartDate & "' And  AddDate <='" & EndDate & "'")(0)
				...
				If PauseNum>0 Then
					If RefreshTotalNum>1 and NowNum Mod PauseNum=0 Then
					    R_RS.Close:Set R_RS=Nothing
						 ...
						 .echo "</script>" &vbcrlf
						 .die "<meta http-equiv=""refresh"" content=""" & PauseTime & ";url=RefreshHtmlSave.asp?f=" & f & "&CurrNowNum=" & NowNum+1 & "&ChannelID=" & ChannelID & "&RefreshFlag=Pause&Types=" & Types & "&StartRefreshTime=" & StartRefreshTime & "&R_Sql=" & Server.UrlEncode(R_Sql) & "&RefreshTotalNum=" & RefreshTotalNum & """>"
					End If
			   End If
			Next
			    If Not KS.IsNul(UpdateSql) Then Conn.Execute(UpdateSql):UpdateSql=""
				.echo "<script>"
				.echo "fsohtml.innerHTML='';" & vbCrLf
				.echo "img2.width=400;" & vbCrLf
