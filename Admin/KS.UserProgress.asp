...
<script>
 ...
 {  onscrolls=false;
	new KesionPopup().PopupCenterIframe('<b>查看用户添加的详细记录</b>',"KS.UserProgress.asp?Action=ShowDetail&"+param,750,380,'auto');
 }
</script>
...
搜索指定用户的稿件情况:<input type='text' name='username'>&nbsp;<input type='submit' class='button' value='搜索用户'>
		...
		With Response
		 ...
		 .Write "new KesionPopup().PopupCenterIframe('<b>查看管理员添加的详细记录</b>',""KS.UserProgress.asp?Action=ShowDetail&""+param,750,380,'auto');"
		 .Write "}"
	     .Write " </script>"
		   ...
		   .Write "<table border='0' width='100%' cellspacing='0' cellpadding='0'>" &vbcrlf
		    For K=0 to Ubound(SQL,2)
			 ...
			 .Write "<td height='22' width='200'>" & SQL(1,k) & "</td>" & vbcrlf
				 .Write "<td width='80' align='center'><a href='javascript:ShowDetail(""username=" & RS(0) &"&ChannelID=" & SQL(0,K)&"&Flag=today"");' title='点击查看详情！'><font color=red>" & Conn.Execute("select count(id) from " & SQL(2,K) & Param &" And datediff("&DataPart_D&",AddDate," & SqlNowString & ")=0")(0) & "</font> " & SQL(4,K) & "</a></td>"
				 .Write "<td width='100' align='center'><a href='javascript:ShowDetail(""username=" & RS(0) &"&ChannelID=" & SQL(0,K)&"&Flag=week"");' title='点击查看详情！'><font color=green>" & Conn.Execute("select count(id) from " & SQL(2,K) & Param &" And datediff("&DataPart_W&",AddDate," & SqlNowString & ")=0")(0) & "</font> " & SQL(4,K) & "</a></td>"
				 .Write "<td width='100' align='center'><a href='javascript:ShowDetail(""username=" & RS(0) &"&ChannelID=" & SQL(0,K)&"&Flag=month"");' title='点击查看详情！'><font color=#ff6600>" & Conn.Execute("select count(id) from " & SQL(2,K) & Param &" And datediff("&DataPart_M&",AddDate," & SqlNowString & ")=0")(0) & "</font> " & SQL(4,K) & "</a></td>"
				 .Write "<td width='100' align='center'><a href='javascript:ShowDetail(""username=" & RS(0) &"&ChannelID=" & SQL(0,K)&"&Flag=year"");' title='点击查看详情！'><font color=blue>" & Conn.Execute("select count(id) from " & SQL(2,K) & Param &" And datediff("&DataPart_Y&",AddDate," & SqlNowString & ")=0")(0) & "</font> " & SQL(4,K) & "</a></td>"
			...
			Select Case Flag
			 Case "today"
			  ...
			  If DataBaseType=1 then
			   Param=" And datediff(day,AddDate," & SqlNowString & ")=0"
			  Else
			 ...
			 Case "week"
			  ...
			  If DataBaseType=1 then
			   Param=" And datediff(week,AddDate," & SqlNowString & ")=0"
			  Else
			 ...
			 Case "month"
			  ...
			  If DataBaseType=1 then
			   Param=" And datediff(month,AddDate," & SqlNowString & ")=0"
			  Else
			 ...
			 Case "year"
			  ...
			  If DataBaseType=1 then
			   Param=" And datediff(year,AddDate," & SqlNowString & ")=0"
			  Else
