       ...
       return true
		...
		Sub SaveMsg_0()
			...
			If Not Rs.eof Then
				...
				Numc= Ubound(SQL,2)
				For i=0 To Numc
					Conn.Execute("insert into KS_Message (incept,sender,title,content,sendtime,flag,issend,DelR,DelS) values('"&SQL(0,i)&"','"&KS.C("AdminName")&"','"&Replace(Title,"'","''")&"','"&Replace(Message,"'","''")&"',"&SqlNowString&",0,1,0,0)")
				Next
			End If
			Rs.Close : Set Rs = Nothing
		End Sub
		...
		Sub SaveMsg_1()
			...
			For i=0 To Numc
				SQL = "Select top 1 UserName From KS_User Where UserName = '"&ToUserName(i)&"'"
				Set Rs = Conn.Execute(SQL)
				If Not Rs.eof Then
				Conn.Execute("insert into KS_Message (incept,sender,title,content,sendtime,flag,issend,DelR,DelS) values('"&ToUserName(i)&"','"&KS.C("AdminName")&"','"&Replace(Title,"'","''")&"','"&Replace(Message,"'","''")&"',"&SqlNowString&",0,1,0,0)")
				End If
			Next
			Rs.Close : Set Rs = Nothing
		End Sub
