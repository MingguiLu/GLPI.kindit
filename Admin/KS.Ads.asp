	   ...
	   Sub AdsMain()
         With Response
		   ...
		   .Write "<html>"
			...
			.Write "function AddPlace(id){new KesionPopup().PopupCenterIframe('添加/编辑广告位','KS.Ads.asp?Action=AddPlace&id='+id,480,200,'no')}"
			.Write "</script>"
  ...
  Sub AdwSave()
       ...
       Dim RSObj:set RSObj=server.createobject("adodb.recordset")
		if  KS.ChkClng(request("place"))=0 then
			...
			set RSObj=nothing
		    if  KS.ChkClng(request("place"))=0 then
			  response.write "<script>if(confirm('广告位添加成功,继续添加吗?')){location.href='?action=AddPlace'}else{parent.Ads.location.href='KS.Ads.asp?Action=Adw';parent.closeWindow()}</script>"
			else
			  response.write "<script>alert('广告位修改成功!');parent.Ads.location.href='KS.Ads.asp?Action=Adw';parent.closeWindow();</script>"
			end if
  End Sub
...
<%
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0">
              <tr> 
                <td width="63" align="center" height="23" class="sort"> <font color="#FF0000">广告位ID</font></td>
                <td width="192" height="20" align="center" class="sort">广告位名称</td>
                <td width="62" height="20" align="center" class="sort">宽度</td>
                <td width="58" height="20" align="center" class="sort">高度</td>
                <td width="119" align="center" class="sort">广告位显示方式</td>
                <td width="234" align="center" class="sort">显示与否</td>
                <td width="219" align="center" class="sort">操 作</td>
              </tr>
<%
