...
{   var ids=get_Ids(document.myform);
	if (ids!='')
		new KesionPopup().PopupCenterIframe('发布选中文档','Include/RefreshHtmlSave.Asp?Types=Folder&RefreshFlag=IDS&ID='+ids,530,110,'no')
	else 
		alert('请选择要发布的文档!');
}		
...
Sub DelInfo()
			  ...
			  C_ID=KS.ChkClng(KS.S("ChannelID"))
			   RS.Open "Select * FROM " & KS.C_S(C_ID,2) &" Where Tid='" & KS.G("FolderID") & "'", conn, 1, 1
			  Do While Not RS.Eof
				  '删除评论
             ...
             Loop
 			Set RS = Nothing
			conn.Execute ("Delete From " & KS.C_S(C_ID,2) &" Where Tid='" &  KS.G("FolderID") & "'")
