...
Class Admin_Form
		...
		Sub Total()
		  ...
		  <div style="color:red;padding-left:30px;margin-top:20px">
		   调用说明：前台模板表单如果只是单步表单并且表单不含联动字段和编辑器字段时采用<scrpit>调用,否则采用iframe调用,如果用iframe调用的请适当调整iframe的宽和高!
		  </div>
		   <script>
			function jm_cc(ob)
			{
	  ...
	  <body>
		...
		End If
		 ...
		 %>
		  <script>
			...
			new KesionPopup().PopupCenterIframe("回复表单记录","KS.Form.asp?Action=replay&formid="+formid+"&id=" +id,650,400,'no')
			 }
			</script>
					...
					totalPut = Conn.Execute("Select count(1) From " & TableName)(0)
							...
							End If
							  ...
							  do while not rs.eof
							   ...
							   response.write "<br>操 作：<a href=""?action=delinfo&FormID=" & ID&"&id=" & rs("Id") & """ onclick=""return(confirm('确定删除吗?'))"">删除</a> <a href='KS.Form.asp?action=modifyinfo&FormID=" & id & "&id=" & rs("id") & "'>修改</a> <a href='?action=setstatus&v=1&FormID=" & ID&"&id=" & rs("id") & "' title='设为已读'>已读</a> <a href='?action=setstatus&v=2&FormID=" & ID&"&id=" & rs("id") & "' title='设为采纳'>采纳</a> <a href='?action=setstatus&v=3&FormID=" & ID&"&id=" & rs("id") & "' title='设为垃圾'>垃圾</a> <a href=""javascript:ShowReplay(" & ID& "," & rs("id") & ");"">回复</a>"
							   response.write "</td>"
							   response.write "</tr>" 
		...
		<div style="width:800px;padding-bottom:5px;margin-bottom:5px;overflow-x: auto; height:auto">
		 <%
			...
			.Write "  <td width='40' align='center' nowrap>选择</td>"
			If IsArray(SQL) Then
				For ii=0 To Ubound(SQL,2)
		...
		Sub ReplaySave()
		 ...
		 Set RS=Nothing
		 If ReturnInfo<>"" Then
		 Response.Write "<script>alert('恭喜，提交回复成功！" & ReturnInfo & "');parent.parent.location.reload();</script>"
		 Else
