 ...
 Else
	...
	If IsObject(Xml) Then
	  For Each Node In XML.DocumentElement.SelectNodes("row")
	    ...
	    response.write "<li><a href=""javascript:;"" onclick=""parent.InsertFileFromUp('" & FileName & "'," & KS.GetFieSize(Server.MapPath(FileName)) &"," & Node.SelectSingleNode("@id").text & ",'" & Node.SelectSingleNode("@title").text & "');parent.closeWindow()"" title='附件id:" & Node.SelectSingleNode("@id").text & " &#13;附件名称:" & Node.SelectSingleNode("@title").text & "&#13;下载次数:" & Node.SelectSingleNode("@hits").text & "次&#13;文件名:" & Node.selectsinglenode("@filename").text & "'>" & KS.Gottopic(Node.SelectSingleNode("@title").text,30) &"</a></li>"
	  Next
	End If
End IF
%>
