...
Sub getRelativeDoc(n)
	 ...
	 If KS.ChkClng(Request("ChannelID"))<>0 Then 
	  Param=" Where Verific=1 and ChannelID=" &KS.ChkClng(Request("ChannelID")) 
	 Else
...
Sub GetClassOption()
 ...
 'If ChannelID=0 Then Exit Sub
  KS.Echo Escape(KS.LoadClassOption(ChannelID,false))
End Sub
...
Sub MessageSave()
		...
		if Content="" Then 
		 ...
		 Response.End
		End if
		If Len(KS.LoseHtml(Content))>=500 Then
		 Response.Write("留言内容不能超过500个字!")
		 Response.End
		End If
...
<table border="0">
 ...
 <input type="button" value=" 发 帖 " style="margin-bottom:6px" class="btn" onClick="toPost('<%if Not KS.IsNul(KS.Setting(69)) then%><%=KS.Setting(66)%>/<%End If%>')"><br/>
 <input type="button" value=" 关 闭 " style="margin-bottom:6px" class="btn" onClick="closeWindow()">
 </td>
 </tr>
</table>
<%		  
