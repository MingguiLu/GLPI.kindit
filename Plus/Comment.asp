...
Sub ShowContent()
  ...
  If IsObject(CommentXML) Then
   For Each Node In CommentXML.DocumentElement.SelectNodes("row")
	   ...
	   CommentStr=CommentStr  & " <font color='#999999'>(发表时间： " & Node.SelectSingleNode("@adddate").text &")</font> </td><td><font style='font-size:32px;font-family:Arial Black;color:#EEF0EE'> " & N & "</font> </td>"
	   CommentStr=CommentStr & "</tr>"
...
<%
         Dim k
    GetWriteComment = GetWriteComment & "<style>"
	GetWriteComment = GetWriteComment & "#commentemot{ width: 209px; position: absolute; border: 1px solid #CDCDCD; background: #fff; z-index: 112;padding:1px; display:none}"
	GetWriteComment = GetWriteComment & "#commentemot img{  float: left;margin:1px; display: inline;border:0  }"
	GetWriteComment = GetWriteComment & "#commentemot a{ border: 1px solid #E9E9E9;float: left;margin:1px; display: inline ;}"
	GetWriteComment = GetWriteComment & "#commentemot a:hover{ border: 1px solid #689ACD }"
	GetWriteComment = GetWriteComment & "</style>"

		 GetWriteComment = GetWriteComment & "<table width=""100%"" border=""0"" align=""center"" cellpadding=""0"" cellspacing=""0"" class=""comment_write_table"">"
		...
		GetWriteComment = GetWriteComment & "  <div style=""height:30px;line-height:30px;text-align:left;""><strong>已有 <span style=""color:brown;font-weight:bold"">" & Conn.Execute("Select count(1) From KS_Comment Where ChannelID=" & ChannelID & " And InfoID=" & InfoID)(0) & "</span> 条跟帖</strong><a href=""" & DomainStr &"plus/Comment.asp?ChannelID=" & ChannelID & "&InfoID=" & InfoID & """ style=""color:brown"">(点击查看)</a></div>"
		GetWriteComment = GetWriteComment & "  <div style=""text-align:left;""><img style=""cursor:pointer"" onclick=""$(\'#commentemot\').toggle()"" src=""" & DomainStr &"images/default/facelist.gif"" align=""absmiddle""/> 用户名："
		...
		If KS.C_S(ChannelID,13)="1" Then
		GetWriteComment = GetWriteComment & "&nbsp;认证码：<input id=""cVerifyCode"" name=""VerifyCode"" type=""text"" size=""4""><img style=""cursor:pointer"" src=""" & DomainStr & "plus/verifycode.asp"" onClick=""this.src=\'" & DomainStr & "plus/verifycode.asp?n=\'+ Math.random();"" id=""verifyimg"" align=""absmiddle"">"
		End IF
		If KS.C_S(Channelid,12)=1 Or KS.C_S(Channelid,12)=2 Then
		...
		GetWriteComment = GetWriteComment & "&nbsp;&nbsp;<a href=""" & DomainStr & "?do=reg""><font color=red><u>注册</u><font></a>"
		If KS.C_S(ChannelID,14)<>0  Then
		GetWriteComment = GetWriteComment & "&nbsp;&nbsp;&nbsp;<span style=""color:#999998;font-size:12px"">剩余字数：</span><input disabled type=""text"" id=""cmax"" size=""5"" name=""cmax"" value=""" & KS.C_S(ChannelID,14) & """>"
		End If

		GetWriteComment = GetWriteComment & "  </div>"
		
		
		GetWriteComment = GetWriteComment & "  <div id=""commentemot"" onmouseleave=this.style.display=""none""; >"
		
		 Dim str:str=":)|:(|:D|:(|:@|:o|:P|:$|;P|:L|:Q|:lol|:loveliness:|:funk:|:curse:|:dizzy:|:shutup:|:sleepy:|:hug:|:victory:|:time:|:kiss:|:handshake|:call:|"
		 Dim NS,strArr:strArr=Split(str,"|")
		 For K=1 to 24
		   NS=Right("0" & K,2)
		   GetWriteComment = GetWriteComment & "<a href=""javascript:void(0)""><img style=""cursor:pointer"" title=""" & strarr(k) & """ onclick=""insertface(\'[e" & k &"]\')""  src=""" & DomainStr & "editor/ubb/images/smilies/default/" & NS & ".gif""></a>"
		...
		Else
		GetWriteComment = GetWriteComment & "<textarea style=""font-size:14px;padding:5px;color:#999;border:1px solid #cccccc;width:98%;height:90px;overflow:auto;"" onfocus=""if(this.value==\'文明上网，请对您的发言负责！\'){this.value=\'\'}"" wrap=""PHYSICAL"" onblur=""if(this.value==\'\'){this.value=\'文明上网，请对您的发言负责！\'}"" name=""C_Content"" rows=""4"" id=""C_Content"">文明上网，请对您的发言负责！</textarea>"
		End If
		...
		GetWriteComment = GetWriteComment & "  </tr>"
		GetWriteComment = GetWriteComment & "  <tr>"
		GetWriteComment = GetWriteComment & "    <td height=""22"">&nbsp;&nbsp;<font color=#ff6600>特别声明：发表内容只代表网友个人观点，与本站立场无关。</font>"
		
		GetWriteComment = GetWriteComment & "<input type=""hidden"" name=""sC_Content"" id=""sC_Content""><input type=""submit"" id=""SubmitComment"" name=""SubmitComment"" value=""确认发表"" style=""padding:2px"" onclick=""checkform();return false""/>"
		...
		If KS.ChkClng(KS.C_S(Channelid,12))=0 Then 
		...
		Set RS=Server.CreateObject("ADODB.RECORDSET")
		  ...
		  if KS.ChkClng(KS.C_S(Channelid,12))=1 Or KS.ChkClng(KS.C_S(ChannelID,12))=3 then
			verific=0
		  else
			verific=1
		  end if
